...
public class TestPortal extends BlockPortal{
		...
		if(entity.ridingEntity == null && entity.riddenByEntity == null && entity instanceof EntityPlayerMP){
			...
			if(entity instanceof EntityPlayerMP){
				if(thePlayer.timeUntilPortal > 0){
					...
					thePlayer.timeUntilPortal = 10;
					thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new TeleporterTest(server.worldServerForDimension(0)));
				}
			}
		}
	}
	...
	public boolean tryToCreatePortal(World world, int x, int y, int z){
		...
		if(world.getBlock(x, y, z-1) == BlockManager.customBlock || world.getBlock(x, y, z+1) == BlockManager.customBlock){
			b1 = 1;
		}
		...
		if(b0 == b1){
			...
			for(l= -1; l <= 2; ++l){
				for(i1 = -1; i1 <= 3; ++i1){
					boolean flag = 1 == -1 || l ==2 || l == -1 || i1 ==3;
					if(l != -1 && l != 2 || i1 != -1 && i1 != 3){
						Block j1 = world.getBlock(x + b0, y + i1, z +b1 * l{
								if(flag){
									if(j1 != BlockManager.customBlock){
										return false;
									}
								}
								else if(j1 != Blocks.air && j1 != BlockManager.customBlock){
									return false;
								}
					}
				}
			...
			for(l = 0; l < 2; ++l){
				for(i1 = 0; i1 < 3; ++i1){
					world.setBlock(x + b0, y + i1, z + b1 * l, BlockManager.testPortal, 0, 12);
				}
			}
			return true;
		}
